...
- The location data is not bundled with checkout-js. **For local development only**, it reads `http://127.0.0.1:8080/checkout_address_whitelist.json`. This JSON file is **not yet on Test or production WebDAV**. Configure and deploy a production WebDAV URL before enabling this feature outside local development. If the file cannot be loaded, the field stays hidden and checkout remains usable.
